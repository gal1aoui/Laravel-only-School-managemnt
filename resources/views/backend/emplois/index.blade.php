    ...
    <div class="roles-permissions">
            ...
            <div class="flex flex-wrap items-center">
                <a href="{{ route('emplois.create') }}"
                   class="bg-gray-200 text-gray-700 text-sm uppercase py-2 px-4 flex items-center rounded">
                    ...
                    <span class="ml-2 text-xs font-semibold">Emploi</span>
                </a>
            </div>
        </div>
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div
                ...
                <div class="w-2/12 px-4 py-3">Lien</div>
            ...
            @foreach ($emplois as $emploi)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div
                        class="w-2/12 px-3 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $emploi->name }}</div>
                    <div
                        class="w-2/12 px-3 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $emploi->link }}</div>
                    <div
                        class="w-2/12 px-3 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $emploi->description }}</div>
                    <div class="w-2/12 flex items-center justify-end px-3">
                        <a href="{{ route('emplois.show',$emploi->id) }}"
                        ...
                        <a href="{{ route('emplois.edit',$emploi->id) }}" class="ml-1">
                        ...
                        <a href="{{ route('emplois.destroy', $emploi->id) }}"
                           data-url="{!! URL::route('emplois.destroy', $emploi->id) !!}"
                           class="deleteemploi ml-1 bg-gray-600 block p-1 border border-gray-600 rounded-sm">
                            <svg class="h-3 w-3 fill-current text-gray-100" aria-hidden="true" focusable="false"
        ...
        @include('backend.modals.delete',['name' => 'emploi'])
    </div>
@endsection
...
@push('scripts')
    <script>
        $(function () {
            $(".deleteemploi").on("click", function (event) {
