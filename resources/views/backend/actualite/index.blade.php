...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">Actualites</h2>
            </div>
            <div class="flex flex-wrap items-center">
                <a href="{{ route('actualite.create') }}"
                   class="bg-gray-200 text-gray-700 text-sm uppercase py-2 px-4 flex items-center rounded">
                    ...
                    <span class="ml-2 text-xs font-semibold">Actualite</span>
                </a>
            </div>
        </div>
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div
                ...
                <div class="w-2/12 px-4 py-3">Slug</div>
            ...
            @foreach ($actualite as $actualite)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div
                        class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $actualite->name }}</div>
                    <div
                        class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $actualite->slug }}</div>
                    <div
                        class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $actualite->actualite_code }}</div>
                    <div class="w-3/12 px-4 py-3 text-sm text-gray-600 tracking-tight"><a
                            href="{{ $actualite->description }}">Lien de l'emploi</a></div>
